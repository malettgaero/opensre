...
- Connection pool leaks (exhausting `max_connections`) → `connection_pool_leak` if the leak is on the application side, or `connection_exhaustion` when the server-side ceiling is hit and the application-side cause is not yet pinpointed. High CPU is often just a secondary symptom of accumulated idle sessions. If connections are near 100% AND CPU is near 100%, look for a single shared root cause (a pool leak holding open scan-heavy queries) — do NOT treat them as two independent problems.
  - **`Client:ClientRead` as dominant wait event is the clearest discriminator for a single-root-cause pool leak**: if `Client:ClientRead` leads `top_wait_events` AND connections are near the `max_connections` ceiling AND all load originates from a single service/user, classify as `connection_exhaustion`. The server has finished processing; the client is not reading the results — this is the signature of leaked/accumulated open connections. The concurrent CPU rise is a downstream symptom (accumulated sessions each hold an open scan-heavy query), NOT an independent fault. Do NOT use `dual_resource_exhaustion` in this pattern.
